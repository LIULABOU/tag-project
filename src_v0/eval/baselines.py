...
# clip-only baseline
# ----------------------------------------------------------------------
def run_clip_only(test_dialogues: List[dict], image_map: Dict[str, str], query_mode: str = "mean"):
    clip = CLIPBackbone(device=DEVICE)
    img_proc = clip.processor.image_processor
    tok = clip.processor.tokenizer

    text_vecs = []
    image_vecs = {}        # photo_id -> tensor
    photo_id_order = []
    target_idx = []
...
# ----------------------------------------------------------------------
def run_random(test_dialogues: List[dict], image_map: Dict[str, str]):
    n_queries = 0
    photos = set()
    for ex in test_dialogues:
        photo_id = ex.get("photo_id")
...
# ----------------------------------------------------------------------
# write metrics.json compatible with compare_runs.py
# ----------------------------------------------------------------------
...
def main():
    ...
    if args.mode == "clip":
        ret, n = run_clip_only(test_dialogues, image_map, query_mode=args.query_mode)
        gate_variant = "baseline_clip"
    else:
        ret, n = run_random(test_dialogues, image_map)
    ...
    print(f"\n[{gate_variant}]  dialogues={n}, gallery={ret['N_gallery']}")
