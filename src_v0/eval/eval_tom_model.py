...
# ----------------------------------------------------------------------
# Per-dialogue forward (with optional modality ablation)
# ----------------------------------------------------------------------
@torch.no_grad()
def encode_dialogue(
    clip: CLIPBackbone,
...
def main():
    ...
    if args.ablate_image == "mean" or args.ablate_text == "mean":
        ...
        mean_v_cls, mean_v_patches, mean_h = precompute_modality_means(
            clip, image_map, shard, max_samples=args.mean_max_samples
        )
    ...
    with open(os.path.join(args.out_dir, "per_turn.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["photo_id", "t", "rho", "kl", "a_t", "a_tilde", "d_t", "soft_target"])
        for r in per_turn_records:
            w.writerow([
                r["photo_id"], r["t"], r["rho"], r["kl"],
