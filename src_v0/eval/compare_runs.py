...
# (display_name, dotted_path_into_metrics_json, fmt, lower_is_better_or_None)
COLS = [
    # general
...
def main():
    ...
    # gate variant lookup (informational)
    variants = []
    for m in metrics_list:
        v = (
            m.get("gate_variant")                                       # new format
            or get_path(m, "checkpoint_args.gate_variant")              # new format fallback
            or get_path(m, "args.gate_variant")                         # legacy
        )
