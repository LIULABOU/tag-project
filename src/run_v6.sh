...
#    TURN_WINDOW  full | preveal. default=full. preveal trains on turns before
#                 the first share_photo=True event (before image reveal).
#
#  Multi-seed usage:
...
if [ "$TURN_WINDOW" = "full" ]; then
    ...
    OUT_BASE="outputs/seed_${SEED}_${TURN_WINDOW}"
fi
