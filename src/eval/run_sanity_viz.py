...
def main():
    ...
    ap.add_argument("--rho", type=float, default=0.1,
                    help="fixed repair gate rho_t (prof said 0.1)")
    args = ap.parse_args()
    ...
    align = AlignmentHead(
        h_dim=clip.text_dim,
        v_dim=clip.vision_dim,
        q_dim=clip.vision_dim
    ).to(DEVICE)
    ...
    for batch in dl:
        ...
        ex_dir = os.path.join(
            args.out_dir,
            f"ex_{num_done:02d}_{meta['photo_id'].replace('/', '_')}"
        )
        ...
        # ---------------- Alignment + GRU Tracker ----------------
        with torch.no_grad():
            # Raw evidence
            a_t, e_hat = align(h_all, v_cls, v_patches)   # [T], [T,N]

            # GRU-style tracker
            tracker = FixedGateEvidenceGRU(
                num_patches=num_patches,
                rho=args.rho
            ).to(DEVICE)

            tracker.eval()
            # Final evidence = GRU state
            e_final = tracker(e_hat)   # [T,N]
        ...
        save_alignment_plot(
            a_arr,
            os.path.join(ex_dir, "alignment_trajectory.png")
        )


        # ---------------- Convert to NumPy ----------------
        e_hat_np = e_hat.detach().cpu().numpy().astype(np.float32)
        e_fin_np = e_final.detach().cpu().numpy().astype(np.float32)
        ...
        for t in range(T):
            ...
            hm_hat = (hm_hat - hm_hat.min()) / (
                hm_hat.max() - hm_hat.min() + 1e-9
            )

            overlay_heatmap(
                pil_img,
                hm_hat,
                os.path.join(ex_dir, f"turn_{t:02d}_heatmap_hat.png")
            )
            ...
            hm_fin = (hm_fin - hm_fin.min()) / (
                hm_fin.max() - hm_fin.min() + 1e-9
            )

            overlay_heatmap(
                pil_img,
                hm_fin,
                os.path.join(ex_dir, f"turn_{t:02d}_heatmap_final.png")
            )
